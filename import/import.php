...
if (isset($_POST["import"])) {
    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
        ...
        for ($i = 0; $i <= $sheetCount; $i ++) {
            ...
            if (! empty($product_name) || ! empty($category) || ! empty($im_code) || ! empty($packing) || ! empty($sku_type) || ! empty($vsku_code) || ! empty($base_material) || ! empty($colour) || ! empty($finish) || ! empty($finish_one) || ! empty($style) || ! empty($size) || ! empty($application) || ! empty($application_one) || ! empty($application_areas) || ! empty($application_areas_one) || ! empty($application_areas_two) || ! empty($application_areas_three) || ! empty($application_areas_four) || ! empty($application_areas_five) || ! empty($path) || ! empty($product_image) || ! empty($view) || ! empty($product_multiple_imgs) || ! empty($product_status)) {
                $query = "insert into tiles(`product_name`,`category`,`im_code`,`packing`,`sku_type`,`vsku_code`,`base_material`,`colour`,`finish`,`finish_one`,`style`,`size`,`application`,`application_one`,`application_areas`,`application_areas_one`,`application_areas_two`,`application_areas_three`,`application_areas_four`,`application_areas_five`,`path`,`product_image`,`view`,`product_multiple_imgs`,`product_status`) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
